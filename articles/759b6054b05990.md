...
## メンタルに関しての新しいHealthKitAPI

メンタルヘルスをサポートするためにHealthKitに追加された新しいAPIが追加されました。自分の気分を振り返り、心の状態を記録して精神的な健康状態を追跡できます。また、AppleWatchから`State of Mind`を記録できるため、1日のどの時点でも簡単にチェックできます。

## 追加された評価シート

![](/images/2024-06-15-21-08-21.png)

精神状態を詳しく評価するために、Healthアプリで不安やうつ病に関する標準化されたアンケートに回答することができます。GAD-7およびPHQ-9は質問票であり、世界中の医療従事者がメンタルヘルスのスクリーニングに使用しています。GAD-7（不安症）とPHQ-9（うつ病）に関する評価の結果を読み書きが可能になりました。
...
新しいAPIを使用して、絵文字を使用して気分を記録し、それをHealthKitに保存することができます。まずは記録したい感情を定義します。呼び出しやすいように絵文字をプロパティで定義します。
...
HealthKitでサンプルを検索するための4つの新しい条件指定（Predicate）を導入します。`Kind`・`Valence`・`Labels`・`Associations`を指定してクエリを実行できます。これにより、必要な情報を簡単に取得することが可能です。
...
新しいHealthKitAPIを活用することで、メンタルヘルスの管理ができるようになりました。個人の気分や感情を記録し、詳細な評価を行うことで、精神状態を振り返ることができます。今後、メンタルヘルスの管理をiOSで行う場合は導入していくべきだと感じました。

...
Sampleは複数が存在しており、`HKQuantitySample`に着目して記載します。他のSampleが気にある方は[`HKSampleType`](https://developer.apple.com/documentation/healthkit/hksampletype)をご覧ください。
...
`HealthStore`のメソッドを用いて保存を行う。メソッドには、コールバック関数とSwift Concurrency（async/await）に対応したメソッドが用意されており、どちらを用いても良いです。
...
上記のメソッドの引数に、`2.歩数のデータのSampleを作成する`で作成したSampleを挿入し、保存を実行する。今回は、コールバック関数を用いて実装しています。
...
`HKError.Code.errorAuthorizationDenied`の
エラー出力で失敗します。
